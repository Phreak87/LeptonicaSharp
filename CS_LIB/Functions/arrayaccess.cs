...
if (line == null) {throw new ArgumentNullException  ("line cannot be Nothing");}
	IntPtr linePtr = IntPtr.Zero;
	if (line.GetType() == typeof(IntPtr))
	{
	  linePtr = (IntPtr)line;
	}
	else if (line.GetType() == typeof(Byte[]))
	{ 
	  var cdata = (Byte[])line;
	  linePtr = Marshal.AllocHGlobal(cdata.Length);
	   Marshal.Copy(cdata, 0, linePtr, cdata.Length);
	}
	else if (line.GetType().GetProperty("line") != null)
	{
	   var cdata = (Byte[])line.GetType().GetProperty("data").GetValue(line, null);
	   linePtr = Marshal.AllocHGlobal(cdata.Length);
	   Marshal.Copy(cdata, 0, linePtr, cdata.Length);
	}
