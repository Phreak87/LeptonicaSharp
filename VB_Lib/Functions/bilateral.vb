...
If reduction < 2 or reduction > 16 then
	...
	End if
	Dim _Result as IntPtr = Natives.pixBilateral(pixs.Pointer,   spatial_stdev,   range_stdev,   ncomps,   reduction)
...
If reduction < 2 or reduction > 16 then
	   Throw New ArgumentException ("1, 2 or 4")
	End if
	Dim _Result as IntPtr = Natives.pixBilateralGray(pixs.Pointer,   spatial_stdev,   range_stdev,   ncomps,   reduction)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	...
	Dim _Result as IntPtr = Natives.pixBilateralExact(pixs.Pointer, spatial_kel.Pointer, range_kelPtr)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	...
	Dim _Result as IntPtr = Natives.pixBilateralGrayExact(pixs.Pointer, spatial_kel.Pointer, range_kelPtr)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim _Result as IntPtr = Natives.pixBlockBilateralExact(pixs.Pointer,   spatial_stdev,   range_stdev)
...
Public Shared Function makeRangeKernel(
	...
	Dim _Result as IntPtr = Natives.makeRangeKernel(  range_stdev)
