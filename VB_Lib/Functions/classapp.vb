...
if IsNothing (dirin) then Throw New ArgumentNullException  ("dirin cannot be Nothing")
	...
	Dim _Result as Integer = Natives.jbCorrelation(  dirin,   thresh,   weight,   components,   rootname,   firstpage,   npages,   renderflag)
...
if IsNothing (dirin) then Throw New ArgumentNullException  ("dirin cannot be Nothing")
		if IsNothing (rootname) then Throw New ArgumentNullException  ("rootname cannot be Nothing")
	Dim _Result as Integer = Natives.jbRankHaus(  dirin,   size,   rank,   components,   rootname,   firstpage,   npages,   renderflag)
...
If reduction < 2 or reduction > 16 then
	...
	End if
	Dim pnatlPtr as IntPtr = IntPtr.Zero

	Dim _Result as IntPtr = Natives.jbWordsInTextlines(  dirin,   reduction,   maxwidth,   maxheight,   thresh,   weight, pnatlPtr,   firstpage,   npages)
	
	if pnatlPtr = IntPtr.Zero then pnatl = Nothing else pnatl = new Numa(pnatlPtr)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim pboxadPtr as IntPtr = IntPtr.Zero
	Dim ppixadPtr as IntPtr = IntPtr.Zero
	Dim pnaiPtr as IntPtr = IntPtr.Zero

	Dim _Result as Integer = Natives.pixGetWordsInTextlines(pixs.Pointer,   minwidth,   minheight,   maxwidth,   maxheight, pboxadPtr, ppixadPtr, pnaiPtr)
	
	if pboxadPtr = IntPtr.Zero then pboxad = Nothing else pboxad = new Boxa(pboxadPtr)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	...
	Dim _Result as Integer = Natives.pixGetWordBoxesInTextlines(pixs.Pointer,   minwidth,   minheight,   maxwidth,   maxheight, pboxadPtr, pnaiPtr)
...
if IsNothing (boxa) then Throw New ArgumentNullException  ("boxa cannot be Nothing")
	...
	Dim _Result as IntPtr = Natives.boxaExtractSortedPattern(boxa.Pointer, na.Pointer)
...
if IsNothing (naa1) then Throw New ArgumentNullException  ("naa1 cannot be Nothing")
	...
	Dim _Result as Integer = Natives.numaaCompareImagesByBoxes(naa1.Pointer, naa2.Pointer,   nperline,   nreq,   maxshiftx,   maxshifty,   delx,   dely,   psame,   debugflag)
