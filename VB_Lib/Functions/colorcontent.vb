...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	...
	Dim _Result as Integer = Natives.pixColorContent(pixs.Pointer,   rwhite,   gwhite,   bwhite,   mingray, ppixrPtr, ppixgPtr, ppixbPtr)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim _Result as IntPtr = Natives.pixColorMagnitude(pixs.Pointer,   rwhite,   gwhite,   bwhite,   type)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim _Result as IntPtr = Natives.pixMaskOverColorPixels(pixs.Pointer,   threshdiff,   mindist)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim _Result as IntPtr = Natives.pixMaskOverColorRange(pixs.Pointer,   rmin,   rmax,   gmin,   gmax,   bmin,   bmax)
...
If {32}.contains (pixs.d) = false then Throw New ArgumentException ("32 bpp rgb")
	Dim _Result as Integer = Natives.pixColorFraction(pixs.Pointer,   darkthresh,   lightthresh,   diffthresh,   factor,   ppixfract,   pcolorfract)
...
If {32}.contains (pixs.d) = false then Throw New ArgumentException ("32 bpp rgb")
	...
	Dim pcolormask1Ptr as IntPtr = IntPtr.Zero
	Dim pcolormask2Ptr as IntPtr = IntPtr.Zero
	Dim pixadbPtr as IntPtr = IntPtr.Zero

	Dim _Result as Integer = Natives.pixFindColorRegions(pixs.Pointer, pixmPtr,   factor,   lightthresh,   darkthresh,   mindiff,   colordiff,   edgefract,   pcolorfract, pcolormask1Ptr, pcolormask2Ptr, pixadbPtr)
	
	if pcolormask1Ptr = IntPtr.Zero then pcolormask1 = Nothing else pcolormask1 = new Pix(pcolormask1Ptr)
	if pcolormask2Ptr = IntPtr.Zero then pcolormask2 = Nothing else pcolormask2 = new Pix(pcolormask2Ptr)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim _Result as Integer = Natives.pixNumSignificantGrayColors(pixs.Pointer,   darkthresh,   lightthresh,   minfract,   factor,   pncolors)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim _Result as Integer = Natives.pixColorsForQuantization(pixs.Pointer,   thresh,   pncolors,   piscolor,   debug)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
	Dim _Result as Integer = Natives.pixNumColors(pixs.Pointer,   factor,   pncolors)
...
If {32}.contains (pixs.d) = false then Throw New ArgumentException ("32 bpp rgb")
	...
	Dim pcmapPtr as IntPtr = IntPtr.Zero

	Dim _Result as Integer = Natives.pixGetMostPopulatedColors(pixs.Pointer,   sigbits,   factor,   ncolors,   parrayPtr, pcmapPtr)
	
	ReDim parray(IIf(1 > 0, 1, 1) - 1)
	If parrayPtr <> IntPtr.Zero Then 
	  Marshal.Copy(parrayPtr, parray, 0, parray.count)
	End If
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
If {32}.contains (pixs.d) = false then Throw New ArgumentException ("32 bpp rgb")
	Dim _Result as IntPtr = Natives.pixSimpleColorQuantize(pixs.Pointer,   sigbits,   factor,   ncolors)
...
if IsNothing (pixs) then Throw New ArgumentNullException  ("pixs cannot be Nothing")
If {32}.contains (pixs.d) = false then Throw New ArgumentException ("32 bpp rgb")
	Dim _Result as IntPtr = Natives.pixGetRGBHistogram(pixs.Pointer,   sigbits,   factor)
...
Public Shared Function makeRGBIndexTables(
	...
	Dim _Result as Integer = Natives.makeRGBIndexTables(  prtabPtr,   pgtabPtr,   pbtabPtr,   sigbits)
	
	ReDim prtab(IIf(1 > 0, 1, 1) - 1)
	If prtabPtr <> IntPtr.Zero Then 
	  Marshal.Copy(prtabPtr, prtab, 0, prtab.count)
	End If
	ReDim pgtab(IIf(1 > 0, 1, 1) - 1)
	If pgtabPtr <> IntPtr.Zero Then 
	  Marshal.Copy(pgtabPtr, pgtab, 0, pgtab.count)
	End If
	ReDim pbtab(IIf(1 > 0, 1, 1) - 1)
	If pbtabPtr <> IntPtr.Zero Then 
	  Marshal.Copy(pbtabPtr, pbtab, 0, pbtab.count)
	End If
	return _Result
End Function
...
Public Shared Function getRGBFromIndex(
	...
	Dim _Result as Integer = Natives.getRGBFromIndex(  index,   sigbits,   prval,   pgval,   pbval)
...
Public Shared Function pixHasHighlightRed(
				...
				<Out()> Optional  ByRef pratio as Single = 0f, 
...
If {32}.contains (pixs.d) = false then Throw New ArgumentException ("32 bpp rgb")
	...
	Dim _Result as Integer = Natives.pixHasHighlightRed(pixs.Pointer,   factor,   fract,   fthresh,   phasred,   pratio, ppixdbPtr)
