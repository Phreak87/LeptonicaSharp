...
if IsNothing (fp) then Throw New ArgumentNullException  ("fp cannot be Nothing")
	Dim _Result as IntPtr = Natives.pixReadStreamPng(fp.Pointer)
...
If My.Computer.Filesystem.FileExists (filename) = false then 
	...
	End If
	Dim _Result as Integer = Natives.readHeaderPng(  filename,   pw,   ph,   pbps,   pspp,   piscmap)
...
if IsNothing (fp) then Throw New ArgumentNullException  ("fp cannot be Nothing")
	Dim _Result as Integer = Natives.freadHeaderPng(fp.Pointer,   pw,   ph,   pbps,   pspp,   piscmap)
...
if IsNothing (data) then Throw New ArgumentNullException  ("data cannot be Nothing")
	Dim _Result as Integer = Natives.readHeaderMemPng(  data,   size,   pw,   ph,   pbps,   pspp,   piscmap)
...
if IsNothing (fp) then Throw New ArgumentNullException  ("fp cannot be Nothing")
	...
	Dim _Result as Integer = Natives.fgetPngResolution(fp.Pointer,   pxres,   pyres)
...
If My.Computer.Filesystem.FileExists (filename) = false then 
	   Throw New ArgumentException ("File is missing")
	End If
	Dim _Result as Integer = Natives.isPngInterlaced(  filename,   pinterlaced)
...
if IsNothing (fp) then Throw New ArgumentNullException  ("fp cannot be Nothing")
	...
	Dim _Result as Integer = Natives.fgetPngColormapInfo(fp.Pointer, pcmapPtr,   ptransparency)
...
if IsNothing (filename) then Throw New ArgumentNullException  ("filename cannot be Nothing")
	...
	Dim _Result as Integer = Natives.pixWritePng(  filename, pix.Pointer,   gamma)
...
if IsNothing (fp) then Throw New ArgumentNullException  ("fp cannot be Nothing")
		if IsNothing (pix) then Throw New ArgumentNullException  ("pix cannot be Nothing")
	Dim _Result as Integer = Natives.pixWriteStreamPng(fp.Pointer, pix.Pointer,   gamma)
...
if IsNothing (pix) then Throw New ArgumentNullException  ("pix cannot be Nothing")
	Dim _Result as Integer = Natives.pixSetZlibCompression(pix.Pointer,   compval)
...
if IsNothing (filedata) then Throw New ArgumentNullException  ("filedata cannot be Nothing")
	Dim _Result as IntPtr = Natives.pixReadMemPng(  filedata,   filesize)
...
if IsNothing (pix) then Throw New ArgumentNullException  ("pix cannot be Nothing")
	...
	Dim _Result as Integer = Natives.pixWriteMemPng(  pfiledataPtr,   pfilesize, pix.Pointer,   gamma)
	
	ReDim pfiledata(IIf(pfilesize > 0, pfilesize, 1) - 1)
	If pfiledataPtr <> IntPtr.Zero Then 
	  Marshal.Copy(pfiledataPtr, pfiledata, 0, pfiledata.count)
	End If
	return _Result
End Function
